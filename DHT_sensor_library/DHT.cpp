...
bool DHT::read(bool force) {
  ...
  uint32_t cycles[80];
  {
    ...
    if (expectPulse(LOW) == TIMEOUT) {
      #ifdef DHT_TRACES_ERR_ON
        DEBUG_PRINTLN_ERR(F("DHT timeout waiting for start signal low pulse."));
      #endif
      _lastresult = false;
      return _lastresult;
    }
    if (expectPulse(HIGH) == TIMEOUT) {
      #ifdef DHT_TRACES_ERR_ON
        DEBUG_PRINTLN_ERR(F("DHT timeout waiting for start signal high pulse."));
      #endif
      _lastresult = false;
      return _lastresult;
    }
  ...
  for (int i = 0; i < 40; ++i) {
    ...
    if ((lowCycles == TIMEOUT) || (highCycles == TIMEOUT)) {
      #ifdef DHT_TRACES_ERR_ON
        DEBUG_PRINTLN_ERR(F("DHT timeout waiting for pulse."));
      #endif
      _lastresult = false;
      return _lastresult;
    }
  ...
  if (data[4] == ((data[0] + data[1] + data[2] + data[3]) & 0xFF)) {
    ...
    #ifdef DHT_TRACES_ERR_ON
      DEBUG_PRINTLN_ERR(F("DHT checksum failure!"));
    #endif
    _lastresult = false;
    return _lastresult;
  }
